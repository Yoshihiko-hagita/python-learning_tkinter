...
from src.config.settings import AUTHORITY_MAP, AUTHORITY_REVERSE_MAP
...
class UserFormFrame(BaseFrame):
    def __init__(self, parent, frame_controller):
        ...
        # コンテンツ(右側)-[Widget]
        # =========================
        user_register_frame = tk.Frame(self)
        ...
        # フレーム-[Widget]
        # =========================
        self.user_form_frame = ttk.LabelFrame(user_register_frame)

        self.user_form_frame.pack(fill="both", padx=(5, 5))
        ...
        label_id_important_notes.grid(row=0, column=1, sticky="w", padx=(20, 0))
        # 社員IDラベル
        label_id = tk.Label(
            self.user_form_frame, text="社員ID         :", font=("Meiryo UI", 10)
        )

        label_id.grid(row=1, column=0, sticky="w", padx=(10, 0))
        ...
        label_name_important_notes = tk.Label(
            self.user_form_frame, text="※ (必須)", font=("Meiryo UI", 8), fg="red"
        )
        ...
        # 氏名ラベル
        label_name = tk.Label(
            self.user_form_frame, text="氏 名        　　:", font=("Meiryo UI", 10)
        )

        label_name.grid(row=3, column=0, sticky="w", padx=(10, 0))
        ...
        self.entry_name.grid(row=3, column=1, sticky="w", padx=(20, 0))
        # =========================
        # メールアドレス-[Widget]
        # =========================
        # メール 注意書き
        label_mail_important_notes = tk.Label(
            self.user_form_frame, text="※ (任意)", font=("Meiryo UI", 8), fg="gray"
        )
        ...
        # メールラベル
        label_mail = tk.Label(
            self.user_form_frame, text="メールアドレス   :", font=("Meiryo UI", 10)
        )

        label_mail.grid(row=5, column=0, sticky="w", padx=(10, 0))
        ...
        self.entry_maill.grid(row=5, column=1, sticky="w", padx=(20, 0))
        ...
        label_authority = tk.Label(
            self.user_form_frame, text="権 限          　:", font=("Meiryo UI", 10)
        )

        label_authority.grid(row=7, column=0, sticky="w", padx=(10, 0))
        ...
        self.entry_authority.grid(row=7, column=1, sticky="w", padx=(20, 0))
        ...
        label_password.grid(row=11, column=0, sticky="w", padx=(10, 0), pady=(0, 20))
        ...
        self.entry_password.grid(
            row=11, column=1, sticky="w", padx=(20, 0), pady=(0, 20)
        )
        ...
        label_password_confirm.grid(
            row=12, column=0, sticky="w", padx=(10, 0), pady=(0, 20)
        )
        ...
        self.entry_password_confirm.grid(
            row=12, column=1, sticky="w", padx=(20, 0), pady=(0, 20)
        )
        ...
        self.check_show_password.grid(row=13, column=0, sticky="w", padx=(10, 0))
        ...
        self.button_back = ttk.Button(
            ...
            text="戻る",
            command=lambda: self.frame_controller.show_frame("UserManagementFrame"),
        )
        ...
        self.button_save = ttk.Button(
            register_under_frame, text="保存", command=self.save_user
        )

        self.button_save.grid(column=1, row=0, padx=(0, 50), pady=(30, 15))

        # キャンセルボタン
        self.button_cancel = ttk.Button(
            register_under_frame, text="キャンセル", command=self.clear_form
        )
    ...
    def save_user(self):
        ...
        user = User(
            ...
            authority=AUTHORITY_MAP[self.entry_authority.get()],
        ...
        try:
            ...
            self.clear_form()
            messagebox.showinfo("完了", "ユーザー情報を保存しました。")
            self.frame_controller.show_frame("UserManagementFrame")
    ...
    def set_user_info(self, user):
        ...
        self.entry_authority.set(AUTHORITY_REVERSE_MAP[user["authority"]])
